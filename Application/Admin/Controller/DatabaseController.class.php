...
//    }
    ...
    public function delete($time = 0){
        if($time){
            ...
            if(count(glob($path))){
                ...
                LogTools::activeLog(array('fileName'=>$name));
                $this->success('备份文件删除成功！', Cookie('__forward__'));
            }
        } else {
            $this->error('参数错误！');
        }
    ...
    public function export($tables = null, $id = null, $start = null){
        if(IS_POST && !empty($tables) && is_array($tables)){ //初始化
            ...
            if(!session('?now_backup')){
                session('now_backup',array());
            }
            $now_backup = session('now_backup');
            //备份指定表
            ...
            if(false === $start){ //出错
                ...
                $now_backup[] = $tables[$id];
                session('now_backup',$now_backup);
                if(isset($tables[++$id])){
                    ...
                    LogTools::activeLog($now_backup);
                    session('now_backup', null);
                    $this->success('备份完成！');
                }
            } else {
        ...
        if(is_numeric($time) && is_null($part) && is_null($start)){ //初始化
            ...
            if(false === $start){
                ...
                if(isset($list[++$part])){
                    ...
                    LogTools::activeLog(session('backup_list'));
