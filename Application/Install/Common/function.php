...
function check_dirfile(){
    $items = array(
        array('dir',  '可写', 'success', './Uploads/empImg'),
        array('dir',  '可写', 'success', './Downloads'),
...
function check_func(){
    $items = array(
        ...
        array('socket_create',     '支持', 'success','函数'),
...
function write_config($config, $auth){
    if(is_array($config)){
        ...
        //写入环境标识
        $conf = str_replace('[SYSTEM_SIGN]', $auth, $conf);
        //写入应用配置文件
        if(!IS_WRITE){
...
function create_tables($db, $prefix = ''){
    //读取SQL文件
    $sqlFile = MODULE_PATH . 'Data/install.sql';
...
function register_administrator($db, $prefix, $admin, $auth){
    ...
    $sql = "INSERT INTO `[PREFIX]emp` (`id`,`email`, `password`,`name`, `login_switch`, `img_file`, `dept_id`, `create_time`,`status`,) VALUES " .
           "('1', '[EMAIL]', '[PASS]', '[NAME]', 1, 'noneImg.jpg',1,0,'A')";

    $password = user_md5($admin['password'],$auth);
    $sql = str_replace(
        array('[PREFIX]', '[EMAIL]', '[PASS]', '[NAME]'),
        array($prefix, $admin['email'], $password, $admin['name']),
        $sql);
    //执行sql
    $res = $db->execute($sql);
    show_msg('管理员帐号注册完成！');
...
function update_tables($db, $prefix = ''){
    ...
    //替换表前缀
    $sql = str_replace(" `uits_", " `{$prefix}", $sql);
