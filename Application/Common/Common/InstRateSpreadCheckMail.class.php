...
class InstRateSpreadCheckMail
{
    ...
    public function check($ci_no){
        //读取备份配置
        $this->_warningSpreadConf = C('INT_RATE_WARNING_SPREAD');
        //创建备份文件
        $warningArr = array();   //key:Client no.   ,  value:array['name']-客户名称   array['desc']-数组(描述)
        $Client  = M('Client');
        $clientResult = $Client->field(array('ci_no','name','create_emp','rate_float'))->find($ci_no);
        if(is_array($clientResult) && "" != $clientResult['rate_float']){
            $InstRateSpreadArr = json_decode($clientResult['rate_float'],true);
            foreach ($InstRateSpreadArr as $tenorKey => $ccyItem){
                foreach ($ccyItem as $ccyKey => $rateValue){
                    if(empty($rateValue['is_rate']) && $this->spreadCheck($ccyKey,$rateValue['value'])){
                        $warningArr[$ci_no]['name'] = $clientResult['name'];
                        $warningArr[$ci_no]['desc'][] = "Tenor : ".$tenorKey." , Currency : ".$ccyKey." , Spread value : ".$rateValue['value'];
                    }
                }
            }
        }
    ...
    //利率值检查
    private function spreadCheck($ccy,$rateValue){
        //以$ccy取得配置值
        if(array_key_exists($ccy,$this->_warningSpreadConf)){
            $warningValue = floatval($this->_warningSpreadConf[$ccy]);
        }elseif(array_key_exists('ALL',$this->_warningSpreadConf)){
            $warningValue = floatval($this->_warningSpreadConf['ALL']);
        }
        //对比实际值与配置值
        $rateValue = abs(floatval($rateValue));
        if($rateValue >= $warningValue){
            return true;
        }else{
            return false;
        }
